        ...
        if(TabPilihRawat.getSelectedIndex()==0){
            if(!NoRawat.equals("")){
                if(NoPermintaan.trim().equals("")||DiagnosaKlinis.trim().equals("")){
                    ...
                    param.put("norm",norm);
                    param.put("dokter",Sequel.cariIsi("select dokter.nm_dokter from reg_periksa inner join dokter on reg_periksa.kd_dokter=dokter.kd_dokter where reg_periksa.no_rawat=? ",NoRawat));
                    param.put("poli",Sequel.cariIsi("select poliklinik.nm_poli from reg_periksa inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli where reg_periksa.no_rawat=? ",NoRawat));
            ...
            if(!NoRawat.equals("")){
                if(NoPermintaan.trim().equals("")||DiagnosaKlinis.trim().equals("")){
                    ...
                    param.put("norm",norm);
                    param.put("dokter",Sequel.cariIsi("select dokter.nm_dokter from reg_periksa inner join dokter on reg_periksa.kd_dokter=dokter.kd_dokter where reg_periksa.no_rawat=? ",NoRawat));
                    param.put("poli",Sequel.cariIsi("select bangsal.nm_bangsal from kamar_inap inner join kamar inner join bangsal on kamar_inap.kd_kamar=kamar.kd_kamar and kamar.kd_bangsal=bangsal.kd_bangsal where kamar_inap.no_rawat=? ",NoRawat));                    
                    param.put("parameter","%"+TCari.getText().trim()+"%");     
                    param.put("namars",akses.getnamars());
                    param.put("alamatrs",akses.getalamatrs());
                    param.put("kotars",akses.getkabupatenrs());
