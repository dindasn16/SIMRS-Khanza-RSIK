...
import laporan.DlgDataHAIsRalan;
...
public final class DlgKasirRalan extends javax.swing.JDialog {
    ...
    private PreparedStatement psotomatis,psotomatis2,pskasir,pscaripiutang,psrekening,psralan;
    private ResultSet rskasir,rsrekening,rs2;
    ...
    private void initComponents() {
        ...
        ppDataHAIsRalan = new javax.swing.JMenuItem();
        ...
        ppDataHAIsRalan.setBackground(new java.awt.Color(255, 255, 254));
        ppDataHAIsRalan.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        ppDataHAIsRalan.setForeground(new java.awt.Color(50, 50, 50));
        ppDataHAIsRalan.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        ppDataHAIsRalan.setText("Data HAIs Ralan");
        ppDataHAIsRalan.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ppDataHAIsRalan.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ppDataHAIsRalan.setName("ppDataHAIsRalan"); // NOI18N
        ppDataHAIsRalan.setPreferredSize(new java.awt.Dimension(200, 26));
        ppDataHAIsRalan.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ppDataHAIsRalanBtnPrintActionPerformed(evt);
            }
        });
        MenuInputData.add(ppDataHAIsRalan);
        ...
        TglSakit1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "28-10-2022" }));
        ...
        TglSakit2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "28-10-2022" }));
...
private void MnDataPemberianObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDataPemberianObatActionPerformed
    ...
    private void ppDataHAIsRalanBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppDataHAIsRalanBtnPrintActionPerformed
        if(tabModekasir.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, table masih kosong...!!!!");
            TCari.requestFocus();
        }else{
//            if(tbKasirRalan.getSelectedRow()>-1){
//                if(tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),11).toString().equals("")){
                    DlgDataHAIsRalan hais=new DlgDataHAIsRalan(null,false);
                    hais.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                    hais.setLocationRelativeTo(internalFrame1);
                    hais.emptTeks();
                    hais.isCek();
                    hais.setNoRm(TNoRwCari.getText(),DTPCari1.getDate(),DTPCari2.getDate());
                    hais.tampil();
                    hais.setVisible(true);
//                }
//            }
        }
    }//GEN-LAST:event_ppDataHAIsRalanBtnPrintActionPerformed
                ...
                if(tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),11).toString().equals("")){
                    try {
                        pskasir=koneksi.prepareStatement(
                            "select pasien.no_rkm_medis,pasien.nm_pasien,reg_periksa.no_rawat,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur)as umur,pasien.no_peserta, "+
                            ...
                            "pasien.no_rkm_medis=reg_periksa.no_rkm_medis where reg_periksa.no_rawat=?");

                        try {
                            pskasir.setString(1,tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),11).toString());
                            rs2=pskasir.executeQuery();
                            if(rs2.next()){
                                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                                Map<String, Object> param = new HashMap<>();
                                ...
                                param.put("propinsirs",akses.getpropinsirs());
                                param.put("kontakrs",akses.getkontakrs());
                                param.put("tanggalmasuk",Sequel.cariIsi("select tgl_registrasi from reg_periksa where no_rawat=? order by tgl_registrasi asc limit 1",tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),11).toString()));
                                param.put("poli",tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),4).toString());
                                param.put("carabayar",tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),9).toString());
                                if(akses.getkode().equals("Admin Utama")){
                                    param.put("petugas","Petugas Kasir");
                                }else{
                                    param.put("petugas",Sequel.cariIsi("select petugas.nama from petugas where petugas.nip=?",akses.getkode()));
                                }                                    
                                param.put("emailrs",akses.getemailrs());
                                param.put("logo",Sequel.cariGambar("select setting.logo from setting"));
                                Valid.MyReport("rptSuratPengantarPulangRalan.jasper",param,"::[ Surat Pengantar Pulang ]::");
                                this.setCursor(Cursor.getDefaultCursor());
                            }else{
                                JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu pasien...!!!");
                                tbKasirRalan.requestFocus();
    ...
    */
    ...
    private javax.swing.JMenuItem ppDataHAIsRalan;
