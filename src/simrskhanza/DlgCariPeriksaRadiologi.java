...
//          Sequel.cariIsi("select pasien.tgl_lahir from pasien where pasien.no_rkm_medis=? ",Lahir,NoRM.getText());
            ...
            param.put("perusahaan",Sequel.cariIsi("select perusahaan_pasien.nama_perusahaan from perusahaan_pasien where perusahaan_pasien.kode_perusahaan=?",Sequel.cariIsi("select pasien.perusahaan_pasien from pasien where pasien.no_rkm_medis=?",NoRM.getText())));
            param.put("carabayar",Sequel.cariIsi("select penjab.png_jawab from penjab where kd_pj=?",Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString())));
