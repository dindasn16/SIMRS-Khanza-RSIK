...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[O]Surat Pulang Atas Permintaan Sendiri","[L]Master Template Hasil Radiologi","[I]Frekuensi Penyakit IGD","[I]Hitung TOI","[I]Hitung BTO","[I]Hitung GDR","[I]Hitung NDR","[S]Hapus Data Salah"
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "penyakit_igd='"+tbUser.getValueAt(i,872).toString()+"',"+                            
                    "hitung_toi='"+tbUser.getValueAt(i,873).toString()+"',"+
                    "hitung_bto='"+tbUser.getValueAt(i,874).toString()+"',"+
                    "hitung_gdr='"+tbUser.getValueAt(i,875).toString()+"',"+
                    "hitung_ndr='"+tbUser.getValueAt(i,876).toString()+"',"+
                    "hapus_data_salah='"+tbUser.getValueAt(i,877).toString()+"'");                                                     
            }            
            tampil();
            emptTeks();
        }
                            ...
                            try {
                                ...
                                if(i!= -1){
                                    Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                                        ...
                                        "penyakit_igd='"+tbUser.getValueAt(barisdicopy,872).toString()+"',"+
                                        "hitung_toi='"+tbUser.getValueAt(barisdicopy,873).toString()+"',"+
                                        "hitung_bto='"+tbUser.getValueAt(barisdicopy,874).toString()+"',"+
                                        "hitung_gdr='"+tbUser.getValueAt(barisdicopy,875).toString()+"',"+
                                        "hitung_ndr='"+tbUser.getValueAt(barisdicopy,876).toString()+"',"+
                                        "hapus_data_salah='"+tbUser.getValueAt(barisdicopy,877).toString()+"'");                              
                                }    
                                userdicopy="";
                                copyhakakses="";
                                barisdicopy=-1;
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.surat_pulang_atas_permintaan_sendiri,user.template_hasil_radiologi,user.penyakit_igd,user.hitung_toi,user.hitung_bto,user.hitung_gdr,user.hitung_ndr,user.hapus_data_salah from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
                    ...
                    try {
                        if(rs.getString(1).toLowerCase().contains(TCari.getText().toLowerCase())||
                            ...
                            tabMode.addRow(new Object[]{rs.getString(1),
                               ...
                               rs.getBoolean("penyakit_igd"),                               
                               rs.getBoolean("hitung_toi"),
                               rs.getBoolean("hitung_bto"),                               
                               rs.getBoolean("hitung_gdr"), 
                               rs.getBoolean("hitung_ndr"),                               
                               rs.getBoolean("hapus_data_salah")
                            });
                        }   
                    } catch (Exception e) {
                        tabMode.addRow(new Object[]{rs.getString(1),
